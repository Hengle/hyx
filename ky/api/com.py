...
class Com(Base):
    ...
    def detail(self, pk):
        ...
        return OiClassCommon.objects.get(order=order)
    ...
    def update(self,pk):
        order = int(pk)
        lesson = OiClassCommon.objects.filter(order=order)
        if not lesson.exists():
            lesson = OiClassCommon(order=order)
        else:
            lesson = OiClassCommon.objects.get(order=order)
